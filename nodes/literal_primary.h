    ...
    std::string ToString(int counter) const override {
        switch (type) {
            case LiteralType::Integer:
                return std::string(counter, ' ') + "Integer: " + std::to_string(intValue);
            case LiteralType::Real:
                return std::string(counter, ' ') + "Real: " + std::to_string(realValue);
            case LiteralType::Boolean:
                return std::string(counter, ' ') + "Boolean: " + (boolValue ? "true" : "false");
        }
        return ""; 
    }
};
